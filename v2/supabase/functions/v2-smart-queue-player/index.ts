...
Deno.serve(async (request) => {
  ...
  if (!url || !serviceRoleKey) return json({ ok: false, error: 'Smart Queue player service is not configured' }, 500, origin);
  const body = await request.json().catch(() => null) as Record<string, unknown> | null;
  if (!body || body.action !== 'saveOwnPreference') return json({ ok: false, error: 'Invalid Smart Queue player request' }, 400, origin);
  ...
  try {
    ...
    if (playerError || !player || player.status === 'removed' || !['live', 'open', 'active'].includes(String(player.event?.status || '').toLowerCase()) || String(player.event?.matching_mode || '') !== 'smart_queue') return json({ ok: false, error: 'This player cannot change Smart Queue preference now' }, 403, origin);
    ...
    return json({ ok: false, error: error instanceof Error ? error.message : String((error as Record<string, unknown>)?.message || 'Could not save Smart Queue preference') }, 400, origin);
  }
});
