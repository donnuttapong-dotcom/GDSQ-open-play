...
export async function invokeTestAdmin(supabase, action, payload = {}) {
  ...
  if (error) {
    const normalized = await normalizeEdgeFunctionError(error, 'Could not reach Test Admin. Check your connection and try again.');
    if (eventId && [401, 403].includes(normalized.status)) clearTestAdminSession(eventId);
    throw normalized;
  }
